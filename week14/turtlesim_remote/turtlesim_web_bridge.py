    ...
    try:
        async for msg in ws:
            ...
            if msg_type == "command":
                ...
                bridge.explorer = Planner(algorithm=bridge.auto_algorithm)
            elif msg_type == "auto_on":
                bridge.auto = True
                bridge.explorer = Planner(algorithm=bridge.auto_algorithm)
            elif msg_type == "auto_off":
                bridge.auto = False
                bridge.stop()
            elif msg_type == "set_algorithm":
                algo = data.get("algorithm", "astar")
                bridge.auto_algorithm = algo
                bridge.explorer.set_algorithm(algo)
